...
public final class ValidationUtils {
    ...
    public static ValidationResult validateStudentId(String id) {
        if (id == null || !STUDENT_ID_PATTERN.matcher(id).matches()) {
            return ValidationResult.error(id, "STU123", "Invalid Student ID", List.of("STU001", "STU999"));
        }
    ...
    public static ValidationResult validateName(String name) {
        if (name == null || !NAME_PATTERN.matcher(name).matches()) {
            return ValidationResult.error(name, "Letters/spaces/hyphens", "Invalid Name", List.of("Emma Wilson", "Jean-Pierre"));
        }
    ...
    public static ValidationResult validateEmail(String email) {
        if (email == null || !EMAIL_PATTERN.matcher(email).matches()) {
            return ValidationResult.error(email, "user@example.com", "Invalid Email", List.of("peace@university.edu"));
        }
    ...
    public static ValidationResult validateDate(String date) {
        if (date == null || !DATE_PATTERN.matcher(date).matches()) {
            return ValidationResult.error(date, "YYYY-MM-DD", "Invalid Date", List.of("2024-11-03"));
        }
